...
import { api } from '../lib';
import { Card, Button, Chip, EmptyState, Loading } from '../components/ui';
...
export const Workout = ({ routineId, onFinished, onCancel }: {
  ...
  const currentSets = sets[index] ?? [];
  const isTimeBased = current.targetDurationSec !== null;

  const patch = (setNumber: number, changes: Partial<SetState>) =>
    setSets(prev => prev.map((group, i) =>
      i === index ? group.map(s => (s.setNumber === setNumber ? { ...s, ...changes } : s)) : group,
    ));

  const toggle = (setNumber: number) => {
    const target = currentSets.find(s => s.setNumber === setNumber);
    if (!target) return;

    const willComplete = !target.completed;
    patch(setNumber, { completed: willComplete });
    // セット完了で休憩タイマーを自動で回す。手動だと押し忘れる
    setRest(willComplete ? current.restSec : 0);
  };
  ...
  return (
    <div data-testid="workout-screen">
      ...
      <div className="row" style={{ overflowX: 'auto', marginTop: 'var(--sp-3)' }}>
        {data.exercises.map((e, i) => (
          <Chip
            key={e.routineExerciseId}
            label={`${(sets[i] ?? []).every(s => s.completed) ? '✓ ' : ''}${e.exercise.name}`}
            selected={i === index}
            onClick={() => setIndex(i)}
            testId={`workout-tab-${i}`}
          />
        ))}
      </div>

      <h2 className="section-title" data-testid="workout-exercise-name">
        {index + 1}/{data.exercises.length} {current.exercise.name}
      </h2>

      {current.previousLabel && (
        <div className="muted" data-testid="workout-previous-hint">前回: {current.previousLabel}</div>
      )}
        ...
        <Card>
          ...
          {currentSets.map(s => (
            <div key={s.setNumber} className={`set-row${s.completed ? ' set-row--done' : ''}`}
                 data-testid={`set-row-${index}-${s.setNumber}`}>
              <div style={{ textAlign: 'center' }} className="muted">{s.setNumber}</div>
              <input
                className="set-input" inputMode="decimal" placeholder="自重"
                value={s.weight ?? ''}
                onChange={e => patch(s.setNumber, { weight: e.target.value === '' ? null : Number(e.target.value) })}
                data-testid={`set-weight-${index}-${s.setNumber}`}
              />
              {isTimeBased ? (
                <input
                  className="set-input" inputMode="numeric"
                  value={s.durationSec ?? ''}
                  onChange={e => patch(s.setNumber, { durationSec: e.target.value === '' ? null : Number(e.target.value) })}
                  data-testid={`set-duration-${index}-${s.setNumber}`}
                />
              ) : (
                <input
                  className="set-input" inputMode="numeric"
                  value={s.reps ?? ''}
                  onChange={e => patch(s.setNumber, { reps: e.target.value === '' ? null : Number(e.target.value) })}
                  data-testid={`set-reps-${index}-${s.setNumber}`}
                />
              )}
              <button className="check" role="checkbox" aria-checked={s.completed}
                      onClick={() => toggle(s.setNumber)} data-testid={`set-check-${index}-${s.setNumber}`}>
                {s.completed ? '✓' : ''}
              </button>
            </div>
          ))}
          ...
          <div className="row" style={{ justifyContent: 'space-between', marginTop: 'var(--sp-3)' }}>
            <button className="btn btn--ghost" style={{ width: 'auto' }} data-testid="remove-set"
                    onClick={() => setSets(prev => prev.map((g, i) => (i === index && g.length > 1 ? g.slice(0, -1) : g)))}>
              − セットを減らす
            </button>
            <button className="btn btn--ghost" style={{ width: 'auto' }} data-testid="add-set"
                    onClick={() => setSets(prev => prev.map((g, i) => {
                      if (i !== index) return g;
      ...
      <div className="row" style={{ marginTop: 'var(--sp-4)' }}>
        <Button label="前の種目" variant="secondary" disabled={index === 0}
                onClick={() => setIndex(i => Math.max(0, i - 1))} testId="workout-prev" />
        <Button label="次の種目" variant="secondary" disabled={index === data.exercises.length - 1}
                onClick={() => setIndex(i => Math.min(data.exercises.length - 1, i + 1))} testId="workout-next" />
      </div>
    </div>
  );
};
